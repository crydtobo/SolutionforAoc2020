...
bool Passport::check_hgt(){
	...
	if(hgt.size() > 3){
		...
		if(unit == "cm"){
			h1 = (value >= 150 and value <= 193);
		}else if(unit == "in"){
			h1 = (value >= 59 and value <= 76);
		}else{
			h1 = false;
		}
	}
...
bool Passport :: is_valid(){
	...
	if(f == 0) {
		//std::cout<<"Bad-> f = "<<f<< ": byr:"<<byr<<", iyr:"<<iyr<<", eyr: "<<eyr<<", hgt: "<<hgt<<", hcl:"<<hcl<<", ecl: "<<ecl<<", pid:"<< pid<<std::endl;
		return false;
	}
	else{
		f = check();
		//std::cout<<"Good-> f="<<f<<"->  byr:"<<byr<<", iyr:"<<iyr<<", eyr: "<<eyr<<", hgt: "<<hgt<<", hcl:"<<hcl<<", ecl: "<<ecl<<", pid:"<< pid<<std::endl;
		return f;
	}
}