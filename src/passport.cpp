...
std::vector<Passport> parseInputFromFile(std::fstream &file)
{
	...
	std::vector<Passport> passportData;
...
void Passport::parseKey(std::string &&input)
{
...
void Passport::parseToTask(std::string &&input)
{
	size_t len = input.find(" ");
	while (len != std::string::npos)
	{
		parseKey(input.substr(0, len));
		input.erase(0, len + 1);
		len = input.find(" ");
	}
}
...
{
	return (value >= 150 and value <= 193) ? true : false;
}

bool isCorrectHgtForIn(const int &value)
{
	return (value >= 59 and value <= 76) ? true : false;
}

bool biggerThanThree(const std::string &hgt)
{
	int value;
...
bool Passport::checkHgt()
{
	return (hgt.size() > 3) ? biggerThanThree(hgt) : false;
}

bool Passport::checkHcl()
{
	std::string unit = hcl.substr(0, hcl.size());
	const std::regex pattern("^#[0-9a-f]+");
	return (std::regex_match(unit, pattern)) ? true : false;
}

bool Passport::checkEcl()
{
	bool h1 = false;
...
bool Passport::checkPid()
{
	std::string first = pid.substr(0, 1);
	return (pid.size() == 9) ? true : false;
}
